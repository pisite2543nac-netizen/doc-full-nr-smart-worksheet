...
type Seed={key:string;title:string;subjectCode:string;type:'digital'|'paper';totalScore:number;description:string;instructions:string;questions:any[];answers?:Record<string,any>};
const seeds:Seed[]=[
 {key:'WS-D01',title:'การประเมินความปลอดภัยในห้องปฏิบัติการคอมพิวเตอร์',subjectCode:'20001-1001',type:'digital',totalScore:10,description:'ประเมินความปลอดภัย สายไฟ ถังดับเพลิง ความเสี่ยง และเหตุเครื่องดื่มหกใกล้คอมพิวเตอร์',instructions:'ตอบคำถามให้ครบทุกข้อก่อนส่ง',questions:[
  ...
  {id:'q3',type:'paragraph',prompt:'อธิบายขั้นตอนเมื่อเครื่องดื่มหกใกล้คอมพิวเตอร์',required:true,maxLength:500,pasteDisabled:true}],answers:{q1:{type:'singleChoice',value:'เก็บเข้ารางหรืออุปกรณ์จัดสาย',score:3},q2:{type:'multipleChoice',value:['ถังดับเพลิง','ป้ายทางหนีไฟ','ชุดปฐมพยาบาล'],score:3}}},
 {key:'WS-D02',title:'พื้นฐานระบบเครือข่ายคอมพิวเตอร์',subjectCode:'21900-1005',type:'digital',totalScore:10,description:'LAN, Router, Switch, IP Address, Wi-Fi และการเปรียบเทียบ LAN กับ Wi-Fi',instructions:'เลือกคำตอบและอธิบายให้ครบ',questions:[
  ...
  {id:'q3',type:'paragraph',prompt:'เปรียบเทียบ LAN แบบสายกับ Wi-Fi',required:true,maxLength:500}],answers:{q1:{type:'singleChoice',value:'Switch',score:3}}},
 ...
 {key:'WS-D04',title:'ฝึกเขียนคำสั่งเงื่อนไข if...else',subjectCode:'21910-2010',type:'digital',totalScore:15,description:'เลือกตอบ เติมคำตอบ วิเคราะห์โค้ด และเขียน JavaScript',instructions:'ตอบคำถามและเขียนโค้ดตามโจทย์',questions:[
  ...
  {id:'q3',type:'codeEditor',prompt:'เขียนโปรแกรมรับคะแนนและแสดง Pass เมื่อคะแนน >= 50',required:true,pasteDisabled:true}],answers:{q1:{type:'singleChoice',value:'if',score:3},q2:{type:'shortAnswer',value:'true',score:2}}},
 ...
 {key:'WS-D06',title:'แบบทดสอบพื้นฐานการเขียนโปรแกรมคอมพิวเตอร์',subjectCode:'31910-0004',type:'digital',totalScore:15,description:'ตัวแปร ชนิดข้อมูล ตัวดำเนินการ การอ่านผลลัพธ์โค้ด และขั้นตอนวิธี',instructions:'ตอบคำถามให้ครบ',questions:[
  ...
  {id:'q3',type:'paragraph',prompt:'อธิบายความหมายของขั้นตอนวิธี (Algorithm)',required:true,maxLength:500}],answers:{q1:{type:'singleChoice',value:'studentName',score:3},q2:{type:'multipleChoice',value:['string','number','boolean'],score:3}}},
...
async function resolveSubjectId(s:any,overwrite:boolean){const q=await db.collection('subjects').where('code','==',s.code).limit(1).get();if(!q.empty){if(overwrite)await q.docs[0].ref.set({...s,active:true,academicYear:2569,semester:1,updatedAt:FieldValue.serverTimestamp()},{merge:true});return q.docs[0].id;}const ref=db.collection('subjects').doc();await ref.set({...s,active:true,academicYear:2569,semester:1,createdAt:FieldValue.serverTimestamp(),updatedAt:FieldValue.serverTimestamp()});return ref.id;}

export const seedInitialSubjectsAndWorksheets=onCall({region,timeoutSeconds:180},async req=>{
 const a=await requireAdmin(req);const overwrite=(req.data as any)?.overwrite===true;const subjectIds=new Map<string,string>();for(const s of subjects)subjectIds.set(s.code,await resolveSubjectId(s,overwrite));let created=0,skipped=0;
 for(const seed of seeds){const exists=await db.collection('worksheets').where('seedKey','==',seed.key).limit(1).get();if(!exists.empty&&!overwrite){skipped++;continue;}const sub=subjects.find(s=>s.code===seed.subjectCode)!;const ref=exists.empty?db.collection('worksheets').doc():exists.docs[0].ref;const batch=db.batch();batch.set(ref,{seedKey:seed.key,subjectId:subjectIds.get(seed.subjectCode),subjectCode:seed.subjectCode,subjectName:sub.name,subjectColor:sub.colorCode,title:seed.title,type:seed.type,status:'draft',description:seed.description,instructions:seed.instructions,questions:seed.questions,targetClassroomIds:[],targetUserIds:[],allowLate:false,allowResubmit:false,maxAttempts:1,saveDraftEnabled:seed.type==='digital',createdBy:a.uid,createdAt:FieldValue.serverTimestamp(),updatedAt:FieldValue.serverTimestamp()},{merge:true});batch.set(db.doc(`worksheetAnswerKeys/${ref.id}`),{worksheetId:ref.id,totalScore:seed.totalScore,answers:seed.answers||{},updatedAt:FieldValue.serverTimestamp()},{merge:true});await batch.commit();created++;}
 await audit(a,'SEED_INITIAL_DATA','system','seed-v2',{createdWorksheets:created,skipped,overwrite});return {ok:true,subjects:subjects.length,createdWorksheets:created,skipped};
});
