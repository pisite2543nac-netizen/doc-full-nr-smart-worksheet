...
function parseJson(text) {
  ...
  throw new Error(
    "Could not parse Firebase CLI JSON output."
  );
}
...
if (!webApp) {
  console.log(
    "No Firebase Web App found. Creating one..."
  );
...
if (!appId) {
  throw new Error(
    "Firebase Web App ID was not found."
  );
}
...
if (missing.length) {
  throw new Error(
    "Firebase Web SDK config is incomplete: "
    + missing.join(", ")
  );
}

if (sdkConfig.projectId !== projectId) {
  throw new Error(
    `Wrong SDK project: ${sdkConfig.projectId}`
  );
}
...
console.log("OK: Project locked to doc-full-nr.");
